...
//lourity
        if (message.content.length > 25) {
            ...
            setTimeout(async () => {
                await message.channel.send({ content: `${message.author} __bu bir bot id'si değil__, lütfen geçerli bir id gir. (5 saniye sonra silinecek)` }).then((msg) => {
                    setTimeout(async () => {
                        await msg.delete().catch(lourity => { })
                    }, 4000);
                })
            }, 1000);
            return;
...
client.on("interactionCreate", async (interaction) => {
    ...
    const lourity_modal = new ModalBuilder()
    ...
    lourity_modal.addComponents(row);
    ...
    if (interaction.customId === "bot_deny") {
        ...
        await interaction.showModal(lourity_modal);
    }

    if (interaction.customId === "deny_form") {
        const deny_reason = interaction.fields.getTextInputValue("textmenu")
        const add_bot_id = interaction.message.embeds[0].fields[1].value.replace(/```/g, ''); // lourity
...
// developer çıkınca bot otomatik atılma sistemi - lourity
